    ...
    <div class="container-body">
      <div class="configuration">
        ...
        <div>
          ...
          <div class="note">
            Try setting the size to 500px and then check and uncheck this
        ...
        <div>
          ...
          <div class="control-group">
            <div class="control" v-for="event in events" :key="event.sectionEventName">
              <label :for="`${event.sectionEventName}-checkbox`">
                {{
                  event.sectionEventName
                }}
              </label>
              <input
                :id="`${event.sectionEventName}-checkbox`"
                type="checkbox"
        ...
        <button
          class="outline-green"
          @click="addSection"
        >+ add {{ sections.length ? 'another' : 'a' }} section</button>
        <!-- end donut sections -->
      </div>
    </div>
  </div>
  ...
  computed: {
    donutProps() {
      ...
      return {
        background,
        foreground,
        size: computedSize > 0 ? computedSize : 200,
        unit,
        thickness:
          computedThickness >= 0 && computedThickness <= 100
            ? computedThickness
            : 20,
        hasLegend,
        legendPlacement,
        ...
        total: computedTotal > 0 ? computedTotal : 100,
    ...
    listeners() {
      return this.events
        ...
        .reduce(
          (acc, curr) => ({
            ...acc,
            [curr.sectionEventName]: (...args) =>
              this.handleSectionEvent(curr, ...args),
          }),
          {}
        )
    },
